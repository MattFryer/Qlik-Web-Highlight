...
function copyDocs() {
    ...
    return {
        logDocs: {
            task: ['log', 'Copying documentation.']
        },
        readDocs: {
            requires: 'logDocs',
...
function generateDemo(filterCB) {
    ...
    return {
        logDemoStart: {
            task: ['log', 'Generating demo.']
        },

        readLanguages: {
            requires: 'logDemoStart',
            task: ['glob', readArgs]
        },
        filterSnippets: {
            requires: 'readLanguages',
            task: ['filter', filterCB]
        },
        readSnippet: {
            requires: 'filterSnippets',
            task: 'readSnippet'
        },
        templateDemo: {
...
module.exports = function (commander) {
    var hljsExt, output, requiresTask, tasks,
        ...
        templateArgs = {
            template: 'hljs.registerLanguage(' +
                '\'<%= name %>\', <%= content %>);\n',
            skip: 'highlight'
        };
    ...
    tasks = {
        startlog: {
            task: ['log', 'Building highlight.js pack file.']
        },
        read: {
            requires: 'startlog',
            task: ['glob', readArgs]
    ...
    if (commander.compress || commander.target === 'cdn') {
        ...
        tasks.replace2 = {
            ...
            task: ['replaceSkippingStrings', replace(regex.replaces, replaceClassNames)]
        };
